...
export default function AdminProjectsPage() {
  ...
  const deleteProject = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete "${name}"? This will also delete all messages and data for this project. This cannot be undone.`)) return;
    ...
    try {
      ...
      if (!res.ok) {
        alert('Failed to delete: ' + (data.error?.message || data.error || 'Unknown error'));
      } else {
                      ...
                      <tr key={project.id} className="hover:bg-neutral-50 transition-colors">
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              onClick={(e) => { e.preventDefault(); deleteProject(project.id, project.business_name); }}
                              disabled={deleting === project.id}
                              className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-red-50 text-red-600 font-body text-sm font-medium rounded-xl hover:bg-red-100 transition-colors disabled:opacity-50"
                            >
                              {deleting === project.id ? (
